-- pk, fk, unique 제약조건 추가시에 해당 컬럼에 대해 index페이지 자동생성
...
-- 신규 index 생성
create index 인덱스명 on 테이블명(컬럼럼);
create index name_index on author(name);

-- index 1컬럼 뿐만 아니라, 2컬럼을 대상으로 1개의 index를 설정하는 것도 가능
-- 이 경우 두 컬럼을 and조건으로 조회해야만 index를 사용
create index 인덱스명 on 테이블명(컬럼1, 컬럼2);

-- index 성능테스트
-- 기존테이블 삭제 후 간단한 테이블로 index 설정 또는 index 미설정 테스트
