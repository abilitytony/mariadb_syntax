...
select * from author a inner join post p on a_id=p.author_id;
select a.*, p.* from author a inner join post p on a_id=p.author_id;
...
select p.* from post p inner join author a a.id=p.author_id where a.name is not null;
...
-- 서브쿼리 : select문 안에 또 다른 select문을 서브쿼리함
-- where절 안에 서브쿼리
...
select id count (*) from author a left join post p on a.id=p.author_id group by id;
...
-- where와 gruop by
-- 날짜값이 null은 데이터를 제외하고, 날짜별 post 글의 개수 출력.
...
-- group by와 having 
-- having은 gruop by를 통해 나온 집계값에 대한 조건
