...
-- 2)터미널을 열어 select했을 때 위 update 변경사항이 읽히는지 확인(transaction2)
-- 결론: maria DB는 기본이 repeatable read 이므로 dirty read 발생 X.
...
-- repeatable read : 읽기의 일관성 보장 -> lost update 문제 발생 -> 배타적lock(배타적 잠금)으로 해결
